...
def batch_generate_features(csv_file, data_dir, cutoff):
    ...
    if not os.path.exists(f'temp_features/{csv_file.split("/")[-1].split(".")[0]}_{cutoff}_features.csv'):
        print(cutoff)
        print(type(cutoff))
        with Parallel(n_jobs=-1) as parallel:
            features = parallel(delayed(generate_feature)(protein_files[i], ligand_files[i], cutoff) for i in tqdm(range(len(keys))))
        features_df = pd.DataFrame(features)
        features_df.to_csv(f'temp_features/{csv_file.split("/")[-1].split(".")[0]}_{cutoff}_features.csv', index=False)
    else:
        features_df = pd.read_csv(f'temp_features/{csv_file.split("/")[-1].split(".")[0]}_{cutoff}_features.csv')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cutoff', help='Distance cutoff for features', default=12, type=int)
    ...
    if args.train:
        ...
        print(f'Pearson: {pearsonr(results_df["pred"], results_df["pk"])[0]}')
        print(f'Spearman: {spearmanr(results_df["pred"], results_df["pk"])[0]}')
    else:
